...
with st.sidebar:
    ...
    اسعار = st.multiselect("أسعار النفط (دولار/برميل)",
                            [40, 45, 50, 55, 58.5, 60, 62, 65, 67, 70, 72, 75, 80, 90, 100, 110, 120],
    ...
    سعر_الصرف = st.number_input("سعر الصرف (دينار/دولار)", value=1320)
    ...
    نقد = st.number_input("العملة المصدرة (مليار دينار)", value=104000, step=1000)
    نسبة_المركزي = st.slider("نسبة تمويل المركزي للعجز", 0.0, 1.0, 0.3, 0.05)
    st.caption(f"القيمة: {نسبة_المركزي:.0%}")
    نسبة_فائض = st.slider("نسبة الفائض تذهب للاحتياطي", 0.0, 1.0, 0.5, 0.05)
    st.caption(f"القيمة: {نسبة_فائض:.0%}")
...
with col3:
    ...
    plt.plot(df["سعر النفط (دولار)"], df["العملة المصدرة (مليار دينار)"]/1000.0, marker="o", label=ar("العملة المصدرة"))
    plt.plot(df["سعر النفط (دولار)"], df["الفجوة (مليار دينار)"]/1000.0, marker="o", label=ar("الفجوة"))
    plt.title(ar("الاحتياطي، العملة المصدرة، والفجوة (ترليون دينار)"))
    plt.xlabel(ar("سعر النفط (دولار/برميل)"))
    plt.ylabel(ar("ترليون دينار"))
    plt.grid(True)
    plt.legend()
